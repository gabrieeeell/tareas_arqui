...
Descripción:
Esta función se encarga de conectar cada una de las transformaciones de base presentes.
...
def convertor_general(num_original, base_origen, base_destino):
    if base_destino == base_origen:
        ...
        return desde_decimal(int(num_original), base_destino)
    elif base_destino == 10:
        return  a_decimal(num_original, base_origen)
    elif base_destino == 2:
        if base_origen == 8:
            return Octal_a_binario(num_original)
        elif base_origen == 16:
            return Hexa_a_binario(num_original)
    elif base_origen == 16 and base_destino == 8:
        return hexa_a_octal(num_original)
    elif base_origen == 8 and base_destino == 16:
        return octal_a_hexa(num_original)
    elif base_origen == 2:
        return desde_binario_a_hexa_o_octal(num_original, base_destino)
...
def filtro_ascii_valido(numero, base_origen):
    numero_en_decimal = int( a_decimal(numero, base_origen))
...
Posterior a esto se llama a la función convertor_general.
